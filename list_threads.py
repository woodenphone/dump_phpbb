...
def list_board_threads(requests_session, board_id, output_file_path):
    ...
    if not os.path.exists(os.path.basename(output_file_path)):
        os.makedirs(os.path.basename(output_file_path))
...
def main():
    try:
        ...
        board_id = 21
        list_board_threads(requests_session, board_id, output_file_path=os.path.join('debug', 'board21_threads.txt'))
