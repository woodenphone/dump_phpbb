...
##    userid_path = '#p{pid} > div > div.postbody > p > strong > a'.format(pid=post_id)
    ...
    if ('href' not in userid_html):
        print('No userID for this post! p.outer_html(): {0!r}'.format(p.outer_html()))
        userid = None
    else:
...
def parse_topic(page_html,board_id=None,topic_id=None,offset=None):
